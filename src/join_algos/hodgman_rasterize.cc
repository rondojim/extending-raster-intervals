...
bool RasterGrid::hodgman_rasterize_poly(
      ...
      if (i == ymin_idx) {
        ...
        if (semi_hori_clipped_vertices.size()) {
          ...
          if (cell_code.value == BinaryCellCode::NULL_CODE) {
            // save_vertices_vectors_seg(semi_hori_clipped_vertices_vec,
            // p1_hori, p2_hori,
            ...
            for (std::vector<const Point *> &vec :
                 semi_hori_clipped_vertices_vec) {
              print_vec(vec);
            }
            return false;
          }
        ...
        if (fully_hori_clipped_vertices.size()) {
          ...
          if (cell_code.value == BinaryCellCode::NULL_CODE) {
            // save_vertices_vectors_seg(fully_hori_clipped_vertices_vec,
            // p1_hori, p2_hori, "wrong_hodgman_clip.txt");
            std::cerr << "NUll Cell Code\n";
            std::cout << "Failed in [" << i << ", " << j << "]\n";
            std::cout << "cell polygons: ";
            for (std::vector<const Point *> &vec :
  ...
  // save_clipped_vertices_vectors("hodgman_column_rasterization.txt",
  // i_j_to_vertical_clipped, polygon.minCorner,polygon.maxCorner);
  // save_clipped_vertices_vectors("hodgman_row_rasterization.txt",
