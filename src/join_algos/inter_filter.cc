...
int RasterGrid::k_belongs_in_sequence(double xmin, double xmax, double k, double epsilon) {
    if (step <= 0) {
        std::cerr << "Step must be positive.\n";
        return -1;
    }
...
bool RasterGrid::set_segment_borders_types(const Point &p1, const Point &p2, std::map<std::pair<unsigned int, unsigned int>, RasterCellInfo> &i_j_to_rcell_info) {
  ...
  if (parallelism == 1){
    ...
    if (y_is_on_grid_row){
      ...
      if (std::isnan(on_grid_min_x) || std::isnan(on_grid_max_x)){
        std::cerr << "Error in set_segment_borders_types for parallel in x axis: min_above_or_equal_k or "
    ...
    if (y_is_on_grid_col){
      ...
      if (std::isnan(on_grid_min_y) || std::isnan(on_grid_max_y)){
        std::cerr << "Error in set_segment_borders_types for parallel in y axis: min_above_or_equal_k or "
                    "max_below_or_equal_k returned NaN\n";
        return false;
      }
      ...
      // std::cout << "min_col_idx: " << min_col_idx << ", max_col_idx: " << max_col_idx << ", min_row_idx: " << min_row_idx << ", max_row_idx: " << max_row_idx << std::endl;
...
bool RasterGrid::set_polygon_borders_cell_types(std::vector<const Point*> &vertices,
...
BinaryCellCode RasterGrid::encode(std::vector<std::vector<const Point *>> &vertices_vectors,
